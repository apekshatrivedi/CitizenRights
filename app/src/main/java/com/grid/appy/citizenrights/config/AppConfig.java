package com.grid.appy.citizenrights.config;

/**
 * Created by Appy on 24-Jul-17.
 */

public class AppConfig {

    // Server user login url
    public static String URL_LOGIN = "http://192.168.1.6/grid/login.php";

    // Server user register url
    public static String URL_REGISTER = "http://192.168.1.6/grid/register.php";

    public static String URL_DREGISTER = "http://192.168.1.6/grid/register1.php";

    public static String SERVER_URL = "http://192.168.1.6/grid/upload.php";

    public static String UPLOADMYSQL_URL = "http://192.168.1.6/grid/uploadmysql.php";

    public static String GET_JSON_DATA_HTTP_URL = "http://192.168.1.6/Grid/ImageJsonData.php";

    public static String GET_JSON_DATA_HTTP_URL1 = "http://192.168.1.6/Grid/homejson.php";

    public static String GET_JSON_DATA_HTTP_URL2 = "http://192.168.1.6/Grid/issuehistory.php?useremail=";
            //Tags used in the JSON String


    public static String GET_ISSUE_DATA = "http://192.168.1.6/Grid/issuedetails.php";


}


